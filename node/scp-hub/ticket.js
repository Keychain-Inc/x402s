...
function verifyDirectPayment(header, expect, state) {
  const payload = parsePaymentHeader(header);
  if (!payload) return { ok: false, error: "missing or invalid header" };
  if (payload.scheme !== "statechannel" && payload.scheme !== "statechannel-direct-v1") return { ok: false, error: "wrong scheme" };
  const signingOpts = expect && expect.signingOpts ? expect.signingOpts : undefined;
...
async function verifyPaymentFull(header, options) {
  ...
  // Direct route
  if ((payload.scheme === "statechannel" && payload.route === "direct") || payload.scheme === "statechannel-direct-v1") {
  ...
  if (payload.scheme && payload.scheme !== "statechannel" && payload.scheme !== "statechannel-hub-v1") {
...
function createVerifier({
  ...
  const verify = async (header, invoiceStore) => {
    ...
    const payload = parsePaymentHeader(header);
    if (payload && ((payload.scheme === "statechannel" && payload.route === "direct") || payload.scheme === "statechannel-direct-v1")) {
